...
class SimpleMaskProcessor:
    def __init__(self, vae_scale_factor=8, do_normalize=False, do_binarize=True, do_convert_grayscale=True):
    ...
    def __call__(self, image: Image.Image):
        ...
        # Resize by vae scale factor (e.g., divide dimensions by 8)
        w, h = image.size
        image = image.resize((w // self.vae_scale_factor, h // self.vae_scale_factor), Image.Resampling.LANCZOS)
        ...
        tensor = torch.from_numpy(arr).unsqueeze(0)  # add channel dim for grayscale
        return tensor
...
class AutoMasker:
    def __init__(
        ...
        self.mask_processor = SimpleMaskProcessor(vae_scale_factor=8, do_normalize=False, do_binarize=True, do_convert_grayscale=True)
